...
Codex 会在 `~/.codex/sessions` 和 `~/.codex/archived_sessions` 中写入本地 JSONL rollout 元数据。计量器读取其中的 `token_count` 事件；存在单次增量时直接累加，只有累计总数时则计算相邻事件之间的非负差值。全局模式会在 `~/.codex/token-usage-meter/all-index-v1.json` 保存紧凑索引，其中只有用量数字、模型信息和文件偏移，不包含对话正文。首次索引完成后，每次刷新只读取新追加的内容。
